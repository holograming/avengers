...
export const mergeWorktreeTool: Tool = {
...
export async function handleMergeWorktree(args: Record<string, unknown>) {
  const { taskId, createPR, commitMessage } = args as {
    taskId: string;
    createPR?: boolean;
  ...
  try {
    if (createPR) {
      ...
      const prUrl = execSync(
        `cd ${worktreePath} && git push -u origin ${branchName} && gh pr create --title "${prTitle}" --body "${prBody}"`,
        { encoding: "utf-8" }
      ).trim();
