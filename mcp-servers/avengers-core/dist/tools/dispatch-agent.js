...
import { agentRoles, agentPermissions, assembleAgentPrompt, buildTaskContext } from "../agent-templates.js";
// Valid agent types
const VALID_AGENTS = [
    "captain",
    "ironman",
 ...
 */
function estimateDuration(agent, context) {
...
export async function handleDispatchAgent(args) {
    const params = args;
    const { agent, task, context, worktree = false, priority = "medium", mode = "background", outputFormat = "summary", dependencies = [], acceptanceCriteria = [], constraints = [] } = params;
    // Validate agent
